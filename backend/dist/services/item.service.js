 ...
 */
const createItem = async (itemBody, userId, updatedBy) => {
 ...
 */
const queryItems = async (filter, options, userId) => {
    ...
    let orderBy = { lastUpdated: 'desc' }; // Default sort
    if (sortBy) {
 ...
 * @returns {Promise<Item>}
 */
const updateItemById = async (itemId, updateBody, userId, updatedBy) => {
 ...
 * @returns {Promise<Item>}
 */
const deleteItemById = async (itemId, userId) => {
    const item = await getItemById(itemId, userId);
    if (!item) {
        throw new ApiError(httpStatus.NOT_FOUND, 'Item not found');
    }
