...
import { StockUpdate } from '../generated/prisma/index.js';
 ...
 */
const getStockUpdateById = async (id: string, userId: number): Promise<StockUpdate | null> => {
 ...
 * @returns {Promise<{results: StockUpdate[], page: number, limit: number, totalPages: number, totalResults: number}>}
 */
const getStockUpdatesByItemId = async (
    itemId: string,
    options: {
        limit?: number;
        page?: number;
